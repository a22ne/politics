...
from database.db_setup import Location
...
def populate_districts():
    ...
    added_count = 0
    for county_name, districts in taiwan_data.items():
        # 檢查縣市是否存在
        county_loc = session.query(Location).filter_by(name=county_name, level="county").first()
        if not county_loc:
            county_loc = Location(name=county_name, level="county")
            session.add(county_loc)
            session.flush() # 取得 ID
            
        # 加入行政區
        for district_name in districts:
            dist_loc = session.query(Location).filter_by(name=district_name, level="district", parent_id=county_loc.id).first()
            if not dist_loc:
                session.add(Location(name=district_name, level="district", parent_id=county_loc.id))
                added_count += 1

    session.commit()
    session.close()
    print(f"全台 22 縣市及其所有行政區同步完成！共新增 {added_count} 個區域。")
